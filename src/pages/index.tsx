...
export default function Home() {
  const searchTerm = 'arc browser love';
  ...
  return (
    <div className="p-5 md:p-10 bg-gray-50 dark:bg-black min-h-screen flex flex-col items-center justify-center">
      <h1 className="flex items-center justify-center text-2xl md:text-3xl font-bold mb-5 center text-center dark:text-white">We ❤️ Arc</h1>
      {error && <div className="text-red-500">Error fetching tweets</div>}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-1">
        {tweets.map((tweet: TweetData) => (
          <a className="rounded hover:bg-gray-100 dark:hover:bg-gray-800 h-max bg-gray-50 dark:bg-black p-5 flex flex-col items-center justify-center" key={tweet.id} href={`https://twitter.com/${tweet.userScreenName}/status/${tweet.id}`} target="_blank" rel="noreferrer noopener">
            <div className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-800 p-4 rounded-xl border max-w-xl">
              <div className="flex justify-between">
                <div className="flex items-center">
                  ...
                  <div className="ml-1.5 text-sm leading-tight">
                    <span className="text-black dark:text-white font-bold block ">{tweet.userName}</span>
                    <span className="text-gray-500 dark:text-gray-400 font-normal block">{tweet.userScreenName}</span>
                  </div>
                </div>
              ...
              <p className="text-black dark:text-white block text-xl leading-snug mt-3">{tweet.text}</p>
              <p className="text-gray-500 dark:text-gray-400 text-base py-1 my-0.5">{tweet.createdAt}</p>
